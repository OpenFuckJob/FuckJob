...
export default function MdHContainer(md: MarkdownIt) {
    ...
    md.block.ruler.after("fence", "heading_container", function (state, line, _maxLine) {
        let rg = /^(#+)\s(.*)/;
    ...
    md.renderer.rules["container_div_heading__close"] = function (tokens, idx, _options, _env, slf) {
        return slf.renderToken(tokens, idx, _options);
    };
    (new Array(6).fill(0)).forEach((_, index) => {
        md.renderer.rules[`container_div_${index + 1}_open`] = function (tokens, idx, _options, _env, slf) {
